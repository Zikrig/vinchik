...
- Гео в анкете: GPS **или** текст → поиск по `settlements` → при нескольких одноимённых — кнопки выбора (подпись с соседями); иначе подтверждение «A рядом с B и C». Подписи НП — кириллица (ru/tg), если есть в алиасах. Дамп `data/settlements/settlements.csv.gz` копируется с проектом; пустая таблица → автоимпорт при старте бота.
