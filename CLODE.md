...
| Карточка аккаунта (блоки: премиум / лайки / аккаунт / анкета) | `/accounts/{tg_id}`, `set_account_premium` / `clear_user_likes` / `update_account_*` |
...
- Админка бота `/admin`: заявки по одной со счётчиком (без листания); премиум-юзеры — inline-кнопки с датой + листание; soft-launch; настройки (= веб). Баны — веб.
